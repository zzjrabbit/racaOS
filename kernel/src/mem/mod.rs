...
use ostd::{
    mm::{
        tlb::TlbFlushOp, FrameAllocOptions, PageFlags, PageProperty, UFrame, Vaddr, VmSpace,
        PAGE_SIZE,
    },
    sync::RwMutex,
...
use crate::mem::mapping::VmMapping;
...
impl Vmar {
    pub fn map(&self, addr: Vaddr, size: usize, prop: PageProperty) -> Result<(), Error> {
        ...
        let frames = (0..size / PAGE_SIZE)
            .map(|_| {
                FrameAllocOptions::new()
                    .alloc_frame()
                    .map(|frame| UFrame::from(frame))
            })
            .collect::<Result<Vec<UFrame>, Error>>()?;

        let vm_mapping = VmMapping::new(frames, aligned, size, prop);
