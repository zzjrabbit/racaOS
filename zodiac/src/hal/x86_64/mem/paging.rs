...
pub fn create_new_page_table() -> Arc<RwLock<dyn GeneralPageTable>> {
    let kernel_page_table = kernel_page_table();
    let kernel_page_table = kernel_page_table.read();
    ...
    let mut stack: Vec<(*const PageTable, *mut PageTable, u8)> = alloc::vec![(
        convert_physical_to_virtual(kernel_page_table.physical_address()) as *const _,
        root_table as *mut _,
        4
    )];
