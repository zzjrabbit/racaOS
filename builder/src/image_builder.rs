...
impl ImageBuilder {
    pub fn build(files: BTreeMap<String, PathBuf>, image_path: &Path) -> anyhow::Result<()> {
...
impl FatBuilder {
    pub fn create(files: BTreeMap<String, PathBuf>, out_path: &Path) -> anyhow::Result<()> {
    ...
    pub fn add_files(
        ...
        for (target_path_raw, source) in files {
            let target_path = Path::new(target_path_raw.as_str());
            ...
            let mut new_file = root_dir
                .create_file(target_path_raw.as_str())
