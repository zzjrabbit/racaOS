...
fn find_slp_typa(aml_table: &AmlTable) -> Result<u16, AmlError> {
    ...
    dsdt.parse_table(unsafe {
        let physical_address = aml_table.address;
        let virtual_address = convert_physical_to_virtual(physical_address);
        from_raw_parts(virtual_address as *const u8, aml_table.length as usize)
    })?;
...
impl AmlHandler {
    fn read<T>(&self, address: usize) -> T {
        let address = convert_physical_to_virtual(address);
