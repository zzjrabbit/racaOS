...
use crate::arch::gdt::BP_TSS;
use crate::arch::smp::AP_INFO;
...
impl Scheduler {
    ...
    pub fn schedule(&mut self, context: VirtAddr) -> VirtAddr {
        ...
        let mut info = AP_INFO.try_get().unwrap().lock();
        if unsafe{lapic.is_bsp()} {
            BP_TSS.lock().privilege_stack_table[0] = next_thread.kernel_stack.end_address()
        }else {
            info[id as usize].as_mut().unwrap().tss.privilege_stack_table[0] = next_thread.kernel_stack.end_address()
        };
        
        interrupts::without_interrupts(|| unsafe { drop(info);page_table.switch() });
