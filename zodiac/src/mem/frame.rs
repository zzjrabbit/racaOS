...
impl Bitmap {
    ...
    pub fn find_range(&mut self, length: usize, value: bool, start_align: usize) -> Option<usize> {
        ...
        for (index, &byte) in self.0.iter().enumerate() {
            match byte {
                ...
                b if b == byte_match => {
                    if length < Self::BITS && index % start_align == 0 {
                        return Some(index * Self::BITS);
                    }
                    if count == 0 && index % start_align == 0 {
...
impl BitmapFrameAllocator {
    pub fn init(memory_map: &MemoryMapResponse) -> Self {
        ...
        let bitmap_buffer = unsafe {
            let virtual_address = convert_physical_to_virtual(bitmap_address as usize);
    ...
    pub fn allocate_frames(
        &mut self,
        count: usize,
        start_count_align: usize,
    ) -> Option<PhysicalAddress> {
        let index = self.bitmap.find_range(count, true, start_count_align)?;
