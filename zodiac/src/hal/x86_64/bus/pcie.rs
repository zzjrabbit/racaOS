...
use crate::mem::{
    MMUFlags, PageSize, PhysicalAddress, PhysicalMemory, VirtualMemorySpace,
...
impl<'a> PciAccess<'a> {
    ...
    pub fn mmio_address(&self, address: PciAddress, offset: u16) -> VirtAddr {
        ...
        let virtual_address = convert_physical_to_virtual(physical_address as PhysicalAddress);

        let page_size = PageSize::Size4K;
        let pm =
            PhysicalMemory::containing_address(physical_address as PhysicalAddress, 1, page_size);
        let _ = VirtualMemorySpace::new_kernel()
            .cursor(page_size.align_down(virtual_address), page_size)
            .unwrap()
