...
use super::vfs::inode::{Inode, InodeRef};
...
impl Inode for Fat32Dir {
    ...
    fn create(&self, _name: String, _ty: super::vfs::inode::InodeTy) -> Option<InodeRef> {
        None
    }
}
...
impl Inode for Fat32File {
    ...
    fn read_at(&self, offset: usize, buf: &mut [u8]) {
        ref_to_mut(self.file.as_ref())
            ...
            .unwrap();
        let read_size = buf.len().max(self.size());
    ...
    fn size(&self) -> usize {
        ref_to_mut(self.file.as_ref()).extents().count() * self.cluster_size
    }
}
